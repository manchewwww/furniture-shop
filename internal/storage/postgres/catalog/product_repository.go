...
func NewProductRepository(db *gorm.DB) storage.ProductRepository { return &ProductRepository{db: db} }
...
func (r *ProductRepository) Update(ctx context.Context, id uint, p ec.Product) error {
    p.ID = 0
    return r.db.WithContext(ctx).Model(&ec.Product{}).Where("id = ?", id).Updates(p).Error
}
