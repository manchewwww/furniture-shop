...
type Handler struct { svc service.AuthService }

func NewAuthHandler(svc service.AuthService) *Handler { return &Handler{svc: svc} }
...
func (h *Handler) Register() fiber.Handler {
    return func(c *fiber.Ctx) error {
        ...
        if in.Email == "" || in.Password == "" || in.Name == "" { return c.Status(400).JSON(fiber.Map{"message":"name, email and password are required"}) }
        user := eu.User{Role: "client", Name: in.Name, Email: in.Email, Address: in.Address, Phone: in.Phone}
        if err := user.SetPassword(in.Password); err != nil { return c.Status(500).JSON(fiber.Map{"message": "password hashing failed"}) }
        if err := h.createUser(c.Context(), &user); err != nil { return c.Status(400).JSON(fiber.Map{"message": "could not create user"}) }
...
func (h *Handler) createUser(ctx context.Context, u *eu.User) error { return h.svc.CreateUser(ctx, u) }
...
func (h *Handler) Login() fiber.Handler {
    return func(c *fiber.Ctx) error {
        ...
        if err != nil { return c.Status(401).JSON(fiber.Map{"message": "invalid email or password"}) }
        token, _ := h.svc.GenerateJWT(user)
        return c.JSON(fiber.Map{"token": token, "user": fiber.Map{"id": user.ID, "name": user.Name, "email": user.Email, "role": user.Role}})
    }
}
