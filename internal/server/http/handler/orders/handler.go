...
type Handler struct { svc service.OrdersService }

func NewOrdersHandler(svc service.OrdersService) *Handler { return &Handler{svc: svc} }
...
func (h *Handler) CreateOrder() fiber.Handler {
    return func(c *fiber.Ctx) error {
        ...
        for _, it := range in.Items { items = append(items, service.CreateOrderItem{ProductID: it.ProductID, Quantity: it.Quantity, Options: it.Options}) }
